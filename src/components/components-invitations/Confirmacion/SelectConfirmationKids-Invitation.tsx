...
export default function SelectConfirmationKidsInvitation({ label, placeholder = "Selecciona", selectedOption, onSelect, persons }: SelectProps) {
    // Generar opciones dinámicamente según el número de personas
    const opcionesDinamicas = persons
        ? Array.from({ length: persons }, (_, i) => ({
            id: i + 1,
            label: `${i + 1} Menor${i + 1 > 1 ? 'es' : ''}`,
            value: (i + 1).toString(),
        }))
        : [];
