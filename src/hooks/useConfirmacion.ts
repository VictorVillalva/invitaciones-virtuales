...
export const useConfirmacion = ({ params, datos }: ConfirmacionProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (hasKids) {
            ...
            if (selectedOption === "") {
                setHeaderError("Olvidaste confirmar");
                setError("Por favor, selecciona la cantidad de invitados que asistiran a la fiesta.");
                setIsModalOpen(true);
                return;
            }
        }
...
export const useConfirmacion2 = ({ params, datos }: ConfirmacionProps) => {
    const [hasKids, setHasKids] = useState(false);
    const [selectedOption, setSelectedOption] = useState("");
    const [selectedKidsOption, setSelectedKidsOption] = useState("")
    const [message, setMessage] = useState("");
...
export const useConfirmacionAsistenciaPamela = ({ codeParam }: UseConfirmacionAsistenciaProps) => {
    const [guestsData, setGuestsData] = useState<Guest | null>(null);
